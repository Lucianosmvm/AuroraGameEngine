...
/// <summary>
/// Escritor de PNG mínimo (RGBA, 8 bits, sem entrelaçamento). A engine só sabe <em>ler</em>
/// imagem (StbImageSharp); trazer uma dependência de escrita só pra gravar um print ou um
/// tileset gerado em código seria exagero — o formato cabe em três blocos: cabeçalho, dados
/// comprimidos com deflate e fim.
/// </summary>
public static class PngWriter
{
    private static readonly byte[] Signature = [137, 80, 78, 71, 13, 10, 26, 10];

    /// <param name="rgba">width*height*4 bytes, linha a linha, de cima pra baixo.</param>
    /// <param name="flipVertically">True quando os pixels vieram do framebuffer de GL (origem embaixo).</param>
    public static void Write(string path, int width, int height, byte[] rgba, bool flipVertically = false)
    {
        if (rgba.Length != width * height * 4)
            throw new ArgumentException($"Esperado {width * height * 4} bytes RGBA, recebido {rgba.Length}.", nameof(rgba));
